...
后续核对见 [本次未完成项与 T04 并行放行](parallel-readiness.md) 和 [手机、直播可行性准备](feasibility-readiness.md)。以下保留最初 T00 的实测历史；另一台 Windows 的 Unity 环境证据和本次 JDK 盘点不回填成当时已经执行的验证。T00 仍未关闭。
